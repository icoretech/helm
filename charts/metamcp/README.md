...
# Recommended for production: store bootstrap users in a Secret (e.g., created by ExternalSecret)
userBootstrap:
  existingSecret:
    name: metamcp-users
    key: config.json

# Dev-only alternative (keeps back-compat): define users directly in values
# users:
...
- `users` (dev-friendly, back-compatible), or
- `userBootstrap.existingSecret` (recommended) pointing to a Secret containing `config.json`.
...
Secret-based bootstrap (recommended):
...
| tolerations | list | `[]` |  |
| userBootstrap.existingSecret.key | string | `"config.json"` |  |
| userBootstrap.existingSecret.name | string | `""` |  |
| users | list | `[]` |  |
